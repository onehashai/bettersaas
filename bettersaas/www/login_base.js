...
login.bind_events = function () {
    ...
    console.log(document.getElementById("login_email"));
    document.getElementById("login_email").onchange=async function(){
        console.log("changed");
    }
	...
	$(".form-login").on("submit", async function (event) {
       ...
       // const hasSubscription = await fetch("/api/method/clientside.clientside.utils.hasActiveSubscription");
        ...
        if(args.usr.toLowerCase() == "administrator"){
            ...
            sitename = ""
        let admin_url = window.location.hostname
        ...
        const r = await getSites(args.usr);
...
async function updateEmail(email){
    ...
    appendSiteOptions(sites.message);
}
